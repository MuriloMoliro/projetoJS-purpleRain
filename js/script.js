...
let update = () => {
    ...
    let rainInitCountnOneFrame = 0
    while(allRains.length < maximumRainCount && maximumRainInitializationInOnFrame > rainInitCountnOneFrame){
        let distanceFromCam = Math.random()
        let rain = new Rain(defaultRainWidth * (2-distanceFromCam),
                            defaultRainHeight * (2-distanceFromCam),
                            (Math.random()/20),
                            Math.random() * canvas.width, -100, (2-distanceFromCam) * 8, 
                            "rgba(197, 55, 230," + ((1-distanceFromCam))+ ")" )

        allRains.push(rain)
        rainInitCountnOneFrame++
    }
